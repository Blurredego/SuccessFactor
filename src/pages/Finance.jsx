...
const Finance = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Registration Report'));
    });
            ...
            <div className="flex flex-col xl:flex-row gap-5 relative">
                ...
                <div className="panel p-0 flex-1">
                    <div className="md:flex items-center flex-wrap p-4 border-b border-[#ebedf2] dark:border-[#191e3a]">
                        ...
                        <ul className="ltr:md:ml-auto rtl:md:mr-auto grid grid-cols-2 sm:grid-cols-3 font-semibold sm:divide-x rtl:divide-x-reverse divide-[#ebedf2] dark:divide-[#253b5c] text-white-dark mt-5 sm:mt-0">
                            <li className="px-4 py-1">
                                In Process
                                <span className="text-lg mt-1.5 block text-black dark:text-white-light">{currentCoin.marketcap}</span>
                            </li>
                            <li className="px-4 py-1">
                                Closed
                                <span className="text-lg mt-1.5 block text-black dark:text-white-light">{currentCoin.volume}</span>
                            </li>
                            <li className="px-4 py-1">
                                Move to Processing
                                <span className="text-lg mt-1.5 block text-black dark:text-white-light">{currentCoin.supply}</span>
                            </li>
                        </ul>
                    </div>
                    <div className="p-4 mb-5 border-b border-[#ebedf2] dark:border-[#253b5c] grid grid-rows-1 sm:grid-cols-4 gap-4">
                        <div>
                            <div className="font-semibold mb-1.5">From</div>
                            <Flatpickr value={date1} options={{ dateFormat: 'Y-m-d', position: isRtl ? 'auto right' : 'auto left' }} className="form-input" onChange={(date) => setDate1(date)} placeholder='Date' />
                        </div>
                        <div>
                            <div className="font-semibold mb-1.5">To</div>
                            <Flatpickr value={date1} options={{ dateFormat: 'Y-m-d', position: isRtl ? 'auto right' : 'auto left' }} className="form-input" onChange={(date) => setDate1(date)} placeholder='Date' />
                        </div>
                        <div>
                            <div className="font-semibold mb-1.5">Users</div>
